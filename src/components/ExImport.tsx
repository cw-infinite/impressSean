...
export function elementToSVG(el: CanvasElement): string {
  ...
  const transform = el.style.rotation ? `rotate(${el.style.rotation} ${cx} ${cy})` : '';
  const opacity = el.style.opacity / 100;
  const strokeAttrs = el.style.borderWidth > 0
    ? `stroke="${el.style.borderColor}" stroke-width="${el.style.borderWidth}" stroke-dasharray="${
        el.style.borderStyle === 'dashed' ? '8,6' : el.style.borderStyle === 'dotted' ? '2,4' : '0'
      }"`
    : '';
  ...
  const anchor = t.typography.textAlign === 'center' ? 'middle' : t.typography.textAlign === 'right' ? 'end' : 'start';
  const textX = t.typography.textAlign === 'center' ? cx : t.typography.textAlign === 'right' ? el.x + el.w : el.x;
...
export function projectToSVGString(project: Project): string {
  const connectorsSVG = project.connections.map(c => {
...
export async function rasterizeProjectToCanvas(project: Project, scale = 2): Promise<HTMLCanvasElement> {
...
const safeFileName = (name: string) => (name || 'project').trim().replace(/[^a-z0-9\-_ ]/gi, '_');
    ...
    reader.onload = () => {
      try {
        ...
        const project = normalizeProject({
          ...
          thumbnailColor: parsed.thumbnailColor || COLORS[Math.floor(Math.random() * COLORS.length)],
          connections: parsed.connections || [],
          comments: parsed.comments || [],
          activity: [
            ...(parsed.activity || []),
            { t: Date.now(), type: 'created', detail: 'Imported from JSON file' },
          ],
        });
        resolve(project);
      } catch (err) {
