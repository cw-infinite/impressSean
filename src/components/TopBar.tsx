...
import { BarChart3, ChevronLeft, Download, FileJson, FileText, Github, ImageIcon, LayoutGrid, MousePointer2, Upload } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { exportProjectAsJSON, exportProjectAsPDF, exportProjectAsPNG } from "./ExImport";
...
export function TopBar({ view, setView, activeProject, saveStatus, onBack, onImportProject }: TopBarProps) {
  ...
  return (
    <div style={{ height: 56, borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', padding: '0 16px', background: '#fff', position: 'sticky', top: 0, zIndex: 20, gap: 12 }}>
      ...
      <div style={{ width: 1, height: 20, background: '#e5e7eb' }} />
      <button onClick={() => importInputRef.current?.click()} disabled={busy} style={{ ...btnGhost, border: '1px solid #e5e7eb', opacity: busy ? 0.5 : 1, cursor: busy ? 'not-allowed' : 'pointer' }}>
...
function ExportMenuItem({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', border: 'none', background: 'transparent', fontSize: 13, color: '#374151', cursor: 'pointer', textAlign: 'left' }}
