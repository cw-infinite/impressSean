	...
	useEffect(() => {
		(async () => {
			...
			if (list && Array.isArray(list)) {
				...
				const seeded = [
					DEFAULT_PROJECT("Marketing site redesign"),
					DEFAULT_PROJECT("Mobile app onboarding"),
				];
				seeded[0].elements = [
					{
						id: uid(),
			...
			{activeProject && view !== "dashboard" && (
				<>
					...
					<button
						...
						style={view === "editor" ? btnActive : btnGhost}
					>
				...
				{TOOLS.map((t) => (
					<button
						...
						style={{
							...
							border: "none",
							cursor: "pointer",
							background: tool === t.id ? "#f3f0ff" : "transparent",
							color: tool === t.id ? "#7C3AED" : "#6b7280",
							display: "flex",
							alignItems: "center",
							justifyContent: "center",
						}}
...
function Reports({ project, allProjects }) {
	...
	const activityByDay = useMemo(() => {
		...
		(project.activity || []).forEach((a) => {
			const d = new Date(a.t).toLocaleDateString("en-US", {
				month: "short",
				day: "numeric",
			});
			days[d] = (days[d] || 0) + 1;
	...
	return (
		<div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
			...
			<p style={{ color: "#6b7280", fontSize: 13, marginBottom: 24 }}>
				Live analytics generated from this project's canvas and activity log.
			</p>
